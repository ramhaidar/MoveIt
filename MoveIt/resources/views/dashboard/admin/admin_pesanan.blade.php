        ...
        <main class="flex-nowrap px-5">
            <div class="row">
                <div class="w-75 container py-2 text-center">
                    <h2>Tidak Ada Riwayat Pesanan Sekarang, Silakan Cek Lagi Nanti.</h2>
                </div>
            ...
            <table class="table table-dark table-striped table-hover table-bordered">
                <thead>
                    <tr class="table-primary">
                        ...
                        <th>Customer [Username|Nama|ID]</th>
                        <th>Nama Driver</th>
                        <th>Nama Penerima</th>
                        <th>Alamat Pickup</th>
                        <th>Alamat Tujuan</th>
                    ...
                    @foreach ($pesanan as $item)
                        @if ($item->is_completed == 1)
                            <tr class="table-light">
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->user->username }}<br>{{ $item->user->name }}<br>{{ $item->user->id }}</td>
                                ...
                                <td>
                                    <form action="{{ route('admin.pesanan.hapus.action') }}" method="POST">
                                        @csrf
                                        <input type="hidden" class="form-control form-control-lg" type="hapusorder" name="hapusorder" value="{{ $item->id }}" required />
                                        <button type="submit" class="btn-link" style="padding: 0;
                                        border: none;
                                        background: none;">Hapus Pesanan</button>
                                    </form>
                                </td>
                            </tr>
                        @endif
